...
import { deleteEmployer } from '../../services/registerAPI';
...
function EmployerListing({ data }) {
    ...
    const fetchDataFromAPI = async (userData) => {
        deleteEmployer(userData.emailAddress)
            .then((data) => {
    ...
    const handleSearch = (event) => {
        ...
        const filteredList = initialUserList.filter(user =>
            user.emailAddress.includes(searchValue.toLowerCase())
        );
        ...
        <div className={classes.userList}>
            ...
            {userList.map(user => (
                <div key={user.id} className={classes.userItem}>
                    <div className={classes.userEmail}>{user.emailAddress}</div>
                    <div
